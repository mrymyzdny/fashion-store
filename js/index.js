...
let displayProducts = (allProductsArray , productContainer , rows , page) => {
    ...
    paginatedProduct.forEach( product => {
        productContainer.insertAdjacentHTML('beforeend' , `<div class="shop-item"> 
        <span class="product-title">${product.title}</span><img  class="product-img"  src="${product.src}" alt="clouth-img">
        <div class="product-detaile"><span class="product-price">${product.price + "  $"}</span><button class="product-btn" onClick=" addToBasketHandler(${product.id})">add to basket </button></div></div>`)
    })
}
    ...
    userBasket.forEach( product => {
        ...
        <div class="basket-item basket-column ">
            ...
            <span class="basket-title">${product.title}</span>
             ...
             <button class="basket-del-btn" onClick=" removeProductFromBasket(${ product.id}) ">Delete</button>
        </div>`)
    })
}
...
// Update basker price 
//  there`s a bug  and I Couldn`t handle it  :(((
